...
def startGame(board,players,player):
    ...
    print(players[player],"has won the toss. So,",players[player]," will start first.")
    print()
...
def playMove(board,players, player):
    ...
    row=int(input("Choose the row where you want to put your bet: ")) 
    column=int(input("Choose the column where you want to put your bet: ")) 
    if player==1:
...
def announceResult(state,states,players):    
    if states[state]=="DRAW":
        ...
        print(players[1], "has won the game. Congratulations!!")
    elif states[state]=="P2-WIN":
        print(players[2], "has won the game. Congratulations!!")
    print()
...
while True:
    ...
    if states[state]=="PLAY":
        ...
        if playMore==1:
            ...
            print("Thanks for playing game! Hope you enjoyed it. :):)")
            break
